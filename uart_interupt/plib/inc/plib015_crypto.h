...
#define IS_CRYPTO_GCM_HASH(VALUE) ((VALUE) < 0x100)
#define IS_CRYPTO_GCM_TAG(VALUE) ((VALUE) < 0x100)
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE uint32_t CRYPTO_GetInitVector(uint32_t idx)
{
	assert_param(IS_CRYPTO_IV(idx));

	return READ_REG(CRYPTO->IV[idx]);
}
...
/**
  * @brief   Получение значения регистра, содержащего хэш GCM
  * @param   idx индекс в массиве [0; 3]
  * @retval  void
  */
__STATIC_INLINE uint32_t CRYPTO_GetGCMHash(uint32_t idx)
{
	assert_param(IS_CRYPTO_GCM_HASH(idx));

	return (uint32_t) READ_REG(CRYPTO->GCM_HASH[idx]);
}
