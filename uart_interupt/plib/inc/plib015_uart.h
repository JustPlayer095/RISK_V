...
#ifndef __PLIB015_UART_H
#define __PLIB015_UART_H
...
#define UART_ITSource_ChangeRI UART_IMSC_RIMIM_Msk      /*!< Изменение состояния линии RI */
#define UART_ITSource_ChangeCTS UART_IMSC_CTSMIM_Msk    /*!< Изменение состояния линии CTS */
#define UART_ITSource_ChangeDCD UART_IMSC_DCDMIM_Msk    /*!< Изменение состояния линии DCD */
#define UART_ITSource_ChangeDSR UART_IMSC_DSRMIM_Msk    /*!< Изменение состояния линии DSR */
...
#define UART_ITSource_All (UART_IMSC_RIMIM_Msk |  \
...
/** @defgroup UART_Flag_Define Флаги работы UART
  * @{
  */
#define UART_Flag_ActiveCTS UART_FR_CTS_Msk    /*!< Флаг активного сигнала на линии CTS */
#define UART_Flag_ActiveDSR UART_FR_DSR_Msk    /*!< Флаг активного сигнала на линии DSR */
#define UART_Flag_ActiveDCD UART_FR_DCD_Msk    /*!< Флаг активного сигнала на линии DSR */
#define UART_Flag_ActiveRI UART_FR_RI_Msk      /*!< Флаг активного сигнала на линии RI */
...
#define UART_Flag_All (UART_FR_CTS_Msk |  \
                       UART_FR_DSR_Msk |  \
                       UART_FR_DCD_Msk |  \
                       UART_FR_RI_Msk |   \
                       UART_FR_BUSY_Msk | \
...
/**
  * @brief   Включение аппаратного управления потоком данных по линии CTS
  * @param   UARTx  Выбор модуля UART, где x=0|1|2|3
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void UART_CTSCmd(UART_TypeDef* UARTx, FunctionalState State)
{
    assert_param(IS_UART_PERIPH(UARTx));
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(UARTx->CR_bit.CTSEN, State);
}

/**
  * @brief   Включение аппаратного управления потоком данных по линии RTS
  * @param   UARTx  Выбор модуля UART, где x=0|1|2|3
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void UART_RTSCmd(UART_TypeDef* UARTx, FunctionalState State)
{
    assert_param(IS_UART_PERIPH(UARTx));
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(UARTx->CR_bit.RTSEN, State);
}

/**
  * @brief   Программное управление состоянием линии RTS
  * @param   UARTx  Выбор модуля UART, где x=0|1|2|3
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void UART_SetRTS(UART_TypeDef* UARTx, BitState State)
{
    assert_param(IS_UART_PERIPH(UARTx));
    assert_param(IS_BIT_STATE(State));

    WRITE_REG(UARTx->CR_bit.RTS, State);
}

/**
  * @brief   Программное управление состоянием линии DTR
  * @param   UARTx  Выбор модуля UART, где x=0|1|2|3
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void UART_SetDTR(UART_TypeDef* UARTx, BitState State)
{
    assert_param(IS_UART_PERIPH(UARTx));
    assert_param(IS_BIT_STATE(State));

    WRITE_REG(UARTx->CR_bit.DTR, State);
}
...
/**
  * @brief   Включения режима "петли"
  * @param   UARTx  Выбор модуля UART, где x=0|1|2|3
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void UART_LoopbackCmd(UART_TypeDef* UARTx, FunctionalState State)
{
    assert_param(IS_UART_PERIPH(UARTx));
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(UARTx->CR_bit.LBE, State);
}

/**
  * @brief   Разрешение работы кодека IrDA
  * @param   UARTx  Выбор модуля UART, где x=0|1|2|3
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void UART_SIRCmd(UART_TypeDef* UARTx, FunctionalState State)
{
    assert_param(IS_UART_PERIPH(UARTx));
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(UARTx->CR_bit.SIREN, State);
}

/**
  * @brief   Включение низкопотребляющего режима работы кодека IrDA
  * @param   UARTx  Выбор модуля UART, где x=0|1|2|3
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void UART_SIRLowPowerCmd(UART_TypeDef* UARTx, FunctionalState State)
{
    assert_param(IS_UART_PERIPH(UARTx));
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(UARTx->CR_bit.SIRLP, State);
}

/**
  * @brief   Ручная настройка делителя частоты для пониженного энергопотребления IrDA
  * @param   UARTx  Выбор модуля UART, где x=0|1|2|3
  * @param   Div  Делителя.
  *                  Параметр принимает любое значение из диапазона 1-255.
  * @retval  void
  */
__STATIC_INLINE void UART_SIRDivConfig(UART_TypeDef* UARTx, uint32_t Div)
{
    assert_param(IS_UART_PERIPH(UARTx));
    assert_param(IS_UART_IRDA_DIV(Div));

    WRITE_REG(UARTx->ILPR, Div);
}
