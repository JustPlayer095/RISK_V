...
/** @defgroup FLASH_IRQ Прерывания
  * @{
  */

/**
  * @brief   Разрешение работы прерывания FLASH
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void FLASH_ITCmd(FunctionalState State)
{
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(FLASH->CTRL_bit.IRQEN, State);
}
...
/**
  ...
  * @retval  Status
  */
__STATIC_INLINE FlagStatus FLASH_ITStatus(void)
{
    return (FlagStatus)READ_BIT(FLASH->STAT, FLASH_STAT_IRQF_Msk);
}
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE void FLASH_ITStatusClear(void)
{
    WRITE_REG(FLASH->STAT, FLASH_STAT_IRQF_Msk);
}
