...
#ifndef __PLIB015_SPI_H
#define __PLIB015_SPI_H
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE void SPI_Cmd(SPI_TypeDef* SPIx, FunctionalState State)
{
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE void SPI_SlaveOutputDisCmd(SPI_TypeDef* SPIx, FunctionalState State)
{
    assert_param(IS_SPI_PERIPH(SPIx));
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(SPIx->CR1_bit.SOD, State);
}
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE void SPI_SCKConfig(SPI_TypeDef* SPIx, SPI_SCKPhase_TypeDef SCKPhase, SPI_SCKPolarity_TypeDef SCKPolarity)
{
    assert_param(IS_SPI_PERIPH(SPIx));
    assert_param(IS_SPI_SCK_PHASE(SCKPhase));
    assert_param(IS_SPI_SCK_POLARITY(SCKPolarity));

    MODIFY_REG(SPIx->CR0, SPI_CR0_SPH_Msk | SPI_CR0_SPO_Msk,
...
/**
  ...
  */
__STATIC_INLINE uint32_t SPI_RecieveData(SPI_TypeDef* SPIx)
{
    assert_param(IS_SPI_PERIPH(SPIx));
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE void SPI_ITFIFOLevelRxConfig(SPI_TypeDef* SPIx, uint32_t FIFOLevelRx)
{
    assert_param(IS_SPI_PERIPH(SPIx));
    assert_param(IS_SPI_FIFO_LEVEL(FIFOLevelRx));

    WRITE_REG(SPIx->CR1_bit.RXIFLSEL, FIFOLevelRx);
}
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE void SPI_ITFIFOLevelTxConfig(SPI_TypeDef* SPIx, uint32_t FIFOLevelTx)
{
    assert_param(IS_SPI_PERIPH(SPIx));
    assert_param(IS_SPI_FIFO_LEVEL(FIFOLevelTx));

    WRITE_REG(SPIx->CR1_bit.TXIFLSEL, FIFOLevelTx);
}
...
/**
  ...
  *                  то результат соответсвует логическому ИЛИ их состояний.
  */
__STATIC_INLINE FlagStatus SPI_ITRawStatus(SPI_TypeDef* SPIx, uint32_t ITSource)
{
    assert_param(IS_SPI_PERIPH(SPIx));
    assert_param(IS_SPI_IT_SOURCE(ITSource));

    return (FlagStatus)READ_BIT(SPIx->RIS, ITSource);
}
...
/**
  ...
  *                  то результат соответсвует логическому ИЛИ их состояний.
  */
__STATIC_INLINE FlagStatus SPI_ITMaskedStatus(SPI_TypeDef* SPIx, uint32_t ITSource)
{
    assert_param(IS_SPI_PERIPH(SPIx));
    assert_param(IS_SPI_IT_SOURCE(ITSource));

    return (FlagStatus)READ_BIT(SPIx->MIS, ITSource);
}
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE void SPI_DMARxCmd(SPI_TypeDef* SPIx, FunctionalState State)
{
    assert_param(IS_SPI_PERIPH(SPIx));
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(SPIx->DMACR_bit.RXDMAE, State);
}
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE void SPI_DMATxCmd(SPI_TypeDef* SPIx, FunctionalState State)
{
    assert_param(IS_SPI_PERIPH(SPIx));
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(SPIx->DMACR_bit.TXDMAE, State);
}
